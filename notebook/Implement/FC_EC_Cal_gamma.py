...
#     print(indices)
    # Frequnecy bands
    # delta (1–4 Hz), theta (4–8 Hz), alpha (8–13 Hz), beta (13–30 Hz), and gamma (30–70 Hz).
    conn = spectral_connectivity_epochs(
            data, method=metrics, sfreq=250, indices=indices,
            fmin=30, fmax=70, faverage=True, verbose=False).get_data()[:, 0]
    conn = conn.tolist()
    return conn
...
#         print("The {} trial from {} to {}. ({}sec)".format(ordinal(i+1), d[0], d[1], d[1]-d[0]))
    events = mne.make_fixed_length_events(prepEEG.copy(), duration=30, overlap=0)
    epochs = mne.Epochs(prepEEG, events=events, event_id=None, tmin=0, tmax=30, baseline=None, preload=False, verbose=False)
...
if __name__ == "__main__":
    ...
    for i, d in enumerate(tqdm(EEG_epoches)):
        ...
        np.save('/workspace/brail_poc3/experiments/FC_Result_gamma/{}/{}.npy'.format(metric_Name, EEG[i]['Subject_Number']), FC)
